...
def get_input_data(data, org_dict, item_dict, asset_dict):
    data = data[['ORGANIZATION_CODE', 'ITEM', 'ASSET_NUMBER', 'WEEK',
                 'TRANSACTION_QUANTITY']]
    data['ORGANIZATION_CODE'] += 1
    data['ITEM'] += (1 + len(org_dict))
    data['ASSET_NUMBER'] += (1 + len(org_dict) + len(item_dict))
    data['WEEK'] += (1 + len(org_dict) + len(item_dict) + len(asset_dict))
    data['TRANSACTION_QUANTITY'] *= -1
    inputs = data.iloc[:, :-1]
    total_features = 1 + len(org_dict) + len(item_dict) + len(asset_dict) + 53
