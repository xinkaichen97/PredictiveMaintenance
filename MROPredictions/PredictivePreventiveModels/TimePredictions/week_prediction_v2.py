...
def get_predictions(inputs, model, weeks, counts, cycles, labels=None):
    pred = model.predict(inputs)
    pred_binary = []
    for i in range(len(weeks)):
        week = weeks[i]
        count = counts[i]
        cycle = int(cycles[i].replace('WK', ''))
        count_cycle = int(53 / cycle)
        count_prev = int(count / 4)
        count_thres = count_cycle if count_cycle >= count_prev else count_prev
        outputs = np.sort(pred[i, :])
        count_thres = count_thres + 1 if count_thres < 52 else 52
        thres = outputs[-count_thres]
        first_week = 0
        for j in range(53):
            if pred[i, j] > thres:
                first_week = j
                break
        for j in range(first_week, 53, 2):
            pred[i, j] += thres
        # print(outputs, thres)
